      ...
      <div class="card-body p-4">
        <template v-if="cart.length > 0">
          <ul class="divide-y divide-gray-300">
            <li v-for="(item) in cart" :key="item.id" class="py-2 flex justify-between items-center">
              <div>
                <p class="text-sm font-medium text-gray-700">{{ item.name }}</p>
                <p class="text-xs text-gray-500">Quantity: {{ item.quantity }}</p>
              </div>
              <button class="btn btn-error btn-xs bg-red-500 text-white hover:bg-red-600 px-2 py-1 rounded"
